...
import { RequisitionsApi } from "./api/RequisitionApi";
import {
...
export class NordigenClient {
   ...
   */
  async request<T = unknown, P = unknown>({
    endpoint,
    parameters,
    method = HttpMethod.GET,
  }: RequestWrapperOptions<P>): Promise<T> {
  ...
  async exchangeToken({
    refreshToken,
  }: ExchangeTokenOptions): Promise<SpectacularJWTRefresh> {
