...
import type { HasDetail, WithClient } from "../types/client";
import { RequisitionCreateOptions } from "../types/requisition";
...
export class RequisitionsApi {
  ...
  createRequisition({
    ...
    const payload: RequisitionRequest = {
      ...
      ...(agreement && {
        agreement: typeof agreement === "string" ? agreement : agreement.id,
      }),
      ...(ssn && { ssn: ssn }),
  ...
  getRequisitions({
    limit = 100,
    offset = 0,
  }: RetrieveAllRequisitionsParams = {}): Promise<PaginatedRequisitionList> {
